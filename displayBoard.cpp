...
void displayBoard(int turn, int n, int lbag_size, vector<vector<string>> board, Player *playerlist) {
    ...
    cout << "     " << playerlist[0].name << ": ";
    if (playerlist[0].score == -1) cout << "Resigned\n";
    else cout << playerlist[0].score << endl;
    for (int y = 0; y < 15; y++) {
        ...
        if (y+1 < n) {
            cout << playerlist[y+1].name << ": ";
            if (playerlist[y+1].score == -1) cout << "Resigned\n";
            else cout << playerlist[y+1].score << endl;
        } else if (y == 4) {
